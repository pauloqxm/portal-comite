...
for i, r in enumerate(reservatorios):
    ...
    # Add Vazao_Aloc line if only one reservoir is selected
    if len(reservatorios) == 1 and "Vazao_Aloc" in dfr.columns:
        y_vals_aloc, _ = convert_vazao(dfr["Vazao_Aloc"], unidade_sel)
        fig.add_trace(go.Scatter(x=dfr["Data"], y=y_vals_aloc, mode="lines", name="Vazão Alocada", line=dict(shape="hv", width=2, color="blue"), hovertemplate=f"<b>Vazão Alocada</b><br>Data: %{{x|%d/%m/%Y}}<br>Vazão: %{{y:.3f}} {unit_suffix}<extra></extra>"))
...
st.plotly_chart(fig, use_container_width=True, config={"displaylogo": False}, key="plotly_vazao_evolucao")
    ...
    with st.expander("☰ Estilo do Mapa", expanded=False):
        mapa_tipo = st.selectbox("Selecione o estilo:", ["OpenStreetMap", "Stamen Terrain", "Stamen Toner", "CartoDB positron", "CartoDB dark_matter", "Esri Satellite"], index=0, key="map_style_selector_vazao", label_visibility="collapsed")
